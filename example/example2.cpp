...
void init()
{
    ...
    for (int i = 0; i < arrayNum; i++)
    {
        ...
        if (fp != 0)
        {
            ...
            if (channels == 4 || color_type == PNG_COLOR_TYPE_RGB_ALPHA)
            {
                ...
                data[i] = (png_bytep)malloc(size * sizeof(unsigned char));         
                if (data[i] == NULL)
                {
                ...
                {
                    row_pointers[i] = data[i] + i*rowbytes;
                }
                png_read_image(png_ptr, row_pointers);   
    ...
    if(data[0])
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width[0], height[0], 0, GL_RGBA, GL_UNSIGNED_BYTE, data[0]);
